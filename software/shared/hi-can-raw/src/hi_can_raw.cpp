...
RawCanInterface::RawCanInterface(const string &interface_name)
    : _interface_name(interface_name),
      _socket(_create_socket,
              [this](int socket) { _configure_socket(socket); }) {}
...
void RawCanInterface::transmit(const Packet &packet) {
  ...
  if (frame.len > CAN_MAX_DLEN)
    throw std::runtime_error(
        "Packet data length exceeds maximum CAN frame length");
  ...
  if (bytes_written < 0) {
    ...
    throw std::runtime_error("Failed to transmit entire CAN frame - only " +
                             ...
                             " bytes written of " + std::to_string(frame.len));
}

std::optional<Packet> RawCanInterface::receive(bool blocking) {
  struct can_frame frame{};
  const ssize_t bytes_read =
      recv((int)_socket, &frame, sizeof(frame), blocking ? 0 : MSG_DONTWAIT);
  if (bytes_read < 0) {
  ...
  if (bytes_read != sizeof(frame)) {
    throw std::runtime_error("Failed to read entire CAN frame - only " +
                             std::to_string(bytes_read) + " bytes read of " +
  ...
  const addressing::flagged_address_t address(frame.can_id, is_rtr, is_error,
                                              is_extended);
...
RawCanInterface &
RawCanInterface::add_filter(const addressing::filter_t &address) {
  FilteredCanInterface::add_filter(address);
  _update_filters();
  return *this;
}
RawCanInterface &
RawCanInterface::remove_filter(const addressing::filter_t &address) {
...
void RawCanInterface::_configure_socket(const int &socket) {
  ...
  if (_interface_name != "any") {
    ...
    if (ioctl(socket, SIOCGIFINDEX, &ifr) < 0) {
      string err = std::strerror(errno);
      throw std::runtime_error("Failed to get interface index for \"" +
                               _interface_name + "\": " + err);
    }
  ...
  if (bind(socket, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
    string err = std::strerror(errno);
    throw std::runtime_error("Failed to bind CAN socket on interface \"" +
                             _interface_name + "\": " + err);
  }
}
...
void RawCanInterface::_update_filters() {
  ...
  // apply the filters
  // if (setsockopt((int)_socket, SOL_CAN_RAW, CAN_RAW_FILTER, filters.data(),
