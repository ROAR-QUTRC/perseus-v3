 ...
 *
 * Defined as a macro because we need the using declaration to bring the
...
public:
  ...
  // Since the the transmit/recieve callbacks are probably lambdas which capture
  // `this`, they likely also cannot be copied or moved.
  ...
  typedef std::vector<
      ...
      callback_configs_t;
  /// @brief List of transmission configurations alongside their flagged
  /// addresses
  typedef std::vector<std::pair<addressing::flagged_address_t,
                                PacketManager::transmission_config_t>>
      transmission_configs_t;
  /// @brief Get the @ref PacketManager::callback_config_t "callbacks" for the
  /// data the group manages
  /// @return Group callback configs
  virtual const callback_configs_t &get_callbacks() const { return _callbacks; }
  /// @brief Get the @ref PacketManager::transmission_config_t "transmissions"
  /// the group manages
  /// @return Group transmission configs
  virtual const transmission_configs_t &get_transmissions() const {
...
public:
  virtual void
  deserialize_data(const std::vector<uint8_t> &serialized_data) = 0;
};
...
struct scaled_int32_t : public BidirectionalSerializable {
  ...
  void deserialize_data(const std::vector<uint8_t> &serialized_data) override {
    ...
    std::copy(serialized_data.begin(), serialized_data.end(),
              reinterpret_cast<uint8_t *const>(&raw_data));
    value = static_cast<int32_t>(ntohl(raw_data)) / scaling_factor;
  }
  std::vector<uint8_t> serialize_data() override {
    int32_t raw_data =
        htonl(static_cast<int32_t>(round(value * scaling_factor)));
    std::vector<uint8_t> data_buf;
    data_buf.resize(sizeof(raw_data));
    std::copy_n(reinterpret_cast<uint8_t *const>(&raw_data), sizeof(raw_data),
                data_buf.begin());
    return data_buf;
  }
};
...
struct scaled_int16_t : public BidirectionalSerializable {
  ...
  scaled_int16_t(double _value) : value(_value) {}
  ...
  std::vector<uint8_t> serialize_data() override {
    int16_t raw_data =
        htons(static_cast<int16_t>(round(value * scaling_factor)));
    std::vector<uint8_t> data_buf;
    data_buf.resize(sizeof(raw_data));
    std::copy_n(reinterpret_cast<uint8_t *const>(&raw_data), sizeof(raw_data),
                data_buf.begin());
...
template <typename T> struct wrapped_value_t {
  wrapped_value_t() = default;
  wrapped_value_t(T _value) : value(_value) {}
  T value{};
};
template <typename T>
...
public:
  ...
  SimpleSerializable(const T &value) { static_cast<T &>(*this) = value; }
  ...
  void deserialize_data(const std::vector<uint8_t> &serialized_data) override {
    if (serialized_data.size() != sizeof(T))
      throw std::invalid_argument("Data size does not match");
    std::copy(serialized_data.begin(), serialized_data.end(),
              reinterpret_cast<uint8_t *const>(static_cast<T *>(this)));
  }
  std::vector<uint8_t> serialize_data() override {
    std::vector<uint8_t> data_buf;
    data_buf.resize(sizeof(T));
    std::copy_n(reinterpret_cast<uint8_t *const>(static_cast<T *>(this)),
                sizeof(T), data_buf.begin());
    return data_buf;
  }
};
...
public:
  VescParameterGroup(uint8_t vesc_id,
                     std::chrono::steady_clock::duration transmission_interval =
...
struct _control_t {
  ...
  uint8_t shutdown_timer =
      0; // if a non-0 value is received, shutdown in that many seconds
};
#pragma pack(pop)
...
struct _scheduled_control_t {
  uint8_t bus_off_time =
      0; // if a non-0 value is received, turn off bus in that many seconds
  uint8_t bus_on_time =
      0; // if a non-0 value is received, turn on bus in that many seconds
};
#pragma pack(pop)
typedef SimpleSerializable<_status_t> status_t;
...
public:
  PowerBusParameterGroup(const addressing::legacy::address_t &device_address,
