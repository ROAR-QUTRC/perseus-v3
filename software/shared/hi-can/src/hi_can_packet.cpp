...
Packet::Packet(const flagged_address_t &address, const uint8_t data[],
               size_t data_len) {
  set_address(address);
  set_data(data, data_len);
}
Packet::Packet(const flagged_address_t &address,
...
void Packet::set_data(const uint8_t data[], size_t data_len) {
  if (data_len > MAX_PACKET_LEN)
    throw std::invalid_argument(
        "Data is longer than the maximum packet length");
...
PacketManager::PacketManager(FilteredCanInterface &interface)
  ...
  interface.set_receive_callback(
      [this](const Packet &packet) { this->_handle_received_packet(packet); });
}
...
void PacketManager::handle_receive(bool should_block) {
  ...
  for (auto &[filter, config] : _callbacks) {
    if (!config.has_timed_out &&
        (now - config.last_received >
         (config.config.timeout * MISSED_PACKET_TIMEOUT_COUNT))) {
...
void PacketManager::set_callback(const filter_t &filter,
...
std::optional<PacketManager::callback_config_t>
PacketManager::get_callback(const filter_t &filter) {
...
void PacketManager::set_transmission_config(
    const flagged_address_t &address, const transmission_config_t &config) {
...
void PacketManager::set_transmission_generator(
    const flagged_address_t &address, const data_generator_t &generator) {
  if (const auto it = _transmissions.find(address); it != _transmissions.end())
    it->second.config.generator = generator;
}

void PacketManager::set_transmission_interval(
    const flagged_address_t &address,
    const std::chrono::steady_clock::duration &interval) {
  if (const auto it = _transmissions.find(address); it != _transmissions.end())
    it->second.config.interval = interval;
}

std::optional<PacketManager::transmission_config_t>
PacketManager::get_transmission_config(const flagged_address_t &address) {
  if (const auto it = _transmissions.find(address); it != _transmissions.end())
    return it->second.config;
  return std::nullopt;
}
...
void PacketManager::_handle_received_packet(const Packet &packet) {
  // it should never happen that the packet address is not in the map since
  // we're using a filtered interface, but check anyway to not crash
  if (const std::optional<filter_t> filter =
          _interface.find_matching_filter(packet.get_address());
      filter) {
