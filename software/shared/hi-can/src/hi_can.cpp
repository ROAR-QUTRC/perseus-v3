...
FilteredCanInterface &
FilteredCanInterface::remove_filter(const filter_t &filter) {
  _filters.erase(filter);
  return *this;
}

std::optional<filter_t> FilteredCanInterface::find_matching_filter(
    const flagged_address_t &address) const {
  // because the filters are sorted by most specific mask first, even when
  // multiple filters match, it should generally match the most specific one
  // first
...
bool FilteredCanInterface::address_matches_filters(
    const flagged_address_t &address) const {
