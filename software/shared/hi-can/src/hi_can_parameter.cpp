...
VescParameterGroup::VescParameterGroup(
    uint8_t vesc_id, steady_clock::duration transmission_interval)
    : _vesc_id(vesc_id) {
  ...
  _transmissions.emplace_back(
      ...
      PacketManager::transmission_config_t{
          .generator = [this]() -> auto { return _set_rpm.serialize_data(); },
  ...
  _callbacks.emplace_back(
      filter_t{
          .address = static_cast<flagged_address_t>(
              address_t(vesc_id, command_id::STATUS_1)),
      },
  ...
  _callbacks.emplace_back(
      filter_t{
          .address = static_cast<flagged_address_t>(
              address_t(vesc_id, command_id::STATUS_2)),
      },
      PacketManager::callback_config_t{
          .data_callback =
              [this](const Packet &packet) {
  ...
  _callbacks.emplace_back(
      filter_t{
          .address = static_cast<flagged_address_t>(
              address_t(vesc_id, command_id::STATUS_3)),
      },
      PacketManager::callback_config_t{
          .data_callback =
              [this](const Packet &packet) {
  ...
  _callbacks.emplace_back(
      filter_t{
          .address = static_cast<flagged_address_t>(
              address_t(vesc_id, command_id::STATUS_4)),
      },
      PacketManager::callback_config_t{
          .data_callback =
              [this](const Packet &packet) {
  ...
  _callbacks.emplace_back(
      filter_t{
          .address = static_cast<flagged_address_t>(
              address_t(vesc_id, command_id::STATUS_5)),
      },
      PacketManager::callback_config_t{
          .data_callback =
              [this](const Packet &packet) {
  ...
  _callbacks.emplace_back(
      filter_t{
          .address = static_cast<flagged_address_t>(
              address_t(vesc_id, command_id::STATUS_6)),
      },
      PacketManager::callback_config_t{
          .data_callback =
              [this](const Packet &packet) {
...
EscParameterGroup::EscParameterGroup(
  ...
  const auto speed_address = static_cast<flagged_address_t>(
      address_t(device_address, static_cast<uint8_t>(mcb::groups::ESC),
                static_cast<uint8_t>(esc::parameter::SPEED)));

  _transmissions.emplace_back(
      speed_address,
      PacketManager::transmission_config_t{
          .generator = [this]() -> auto { return _speed.serialize_data(); },
          .interval = 100ms,
          .should_transmit_immediately = false,
      });
  ...
  _callbacks.emplace_back(
      ...
      PacketManager::callback_config_t{
          .data_callback =
              [this](const Packet &packet) {
                _position = SimpleSerializable<wrapped_value_t<int64_t>>(
                                packet.get_data())
                                .value;
              },
      });
...
std::vector<Packet> EscParameterGroup::get_startup_transmissions() const {
  ...
  packets.emplace_back(flagged_address_t{
      address_t(_device_address, static_cast<uint8_t>(mcb::groups::ESC),
                static_cast<uint8_t>(esc::parameter::LIMITS)),
      true,
  });
  return packets;
...
PowerBusParameterGroup::PowerBusParameterGroup(
    const addressing::legacy::address_t &device_address,
  ...
  _callbacks.emplace_back(
      filter_t{
          .address = static_cast<flagged_address_t>(address_t(
              device_address, static_cast<uint8_t>(bus),
              static_cast<uint8_t>(addressing::legacy::power::control::
                                       power_bus::parameter::POWER_STATUS)))},
      PacketManager::callback_config_t{
          .data_callback =
              [this](const Packet &packet) {
                _status.deserialize_data(packet.get_data());
