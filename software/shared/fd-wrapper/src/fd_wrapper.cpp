...
FdWrapper::FdWrapper(function<int(void)> open, function<void(int)> configure,
                     function<void(int)> user_close)
    : _close(user_close) {
  if (!open)
    throw std::invalid_argument("open function must be provided");
  try {
    _fd = open();
    // note: open() should throw an exception if it fails, but if it doesn't,
    // throw one here
    if (_fd < 0)
