...
public:
  FdWrapper() = default;
  FdWrapper(std::function<int(void)> open,
            std::function<void(int)> configure = nullptr,
            std::function<void(int)> close = nullptr);
  ~FdWrapper();

  // we do NOT want to allow copying, that doesn't make sense for file
  // descriptors moving is OK though copy constructor
  FdWrapper(const FdWrapper &) = delete;
  // copy assignment - note that it needs to be a reference, otherwise
  // assignments match for both move and copy
  FdWrapper &operator=(const FdWrapper &) = delete;
  // move constructor
  FdWrapper(FdWrapper &&other) noexcept { swap(*this, other); }
  // move assignment
