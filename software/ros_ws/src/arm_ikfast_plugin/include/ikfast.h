...
public:
  ...
  unsigned char indices[5]; ///< unique index of the solution used to keep track
                            ///< on what part it came from. sometimes a
  /// solution can be repeated for different indices. store at least another
  /// repeated root
};
...
public:
  ...
  virtual void GetSolution(std::vector<T> &solution,
    ...
    GetSolution(&solution.at(0),
                freevalues.size() > 0 ? &freevalues.at(0) : nullptr);
  }
...
public:
  IkFastFunctions()
      : _ComputeIk(nullptr), _ComputeFk(nullptr),
        _GetNumFreeParameters(nullptr), _GetFreeParameters(nullptr),
        _GetNumJoints(nullptr), _GetIkRealSize(nullptr),
        _GetIkFastVersion(nullptr), _GetIkType(nullptr),
