...
{
    FlatFootprintBroadcaster::FlatFootprintBroadcaster(
    ...
    void FlatFootprintBroadcaster::_load_parameters()
    {
        ...
        _base_link_frame =
            this->declare_parameter("base_link_frame", DEFAULT_BASE_LINK_FRAME);
        _base_footprint_frame = this->declare_parameter("base_footprint_frame",
    ...
    void FlatFootprintBroadcaster::_timer_callback()
    {
        ...
        try
        {
            odom_to_base_link = _tf_buffer->lookupTransform(
                _odom_frame, _base_link_frame, tf2::TimePointZero);
        }
        catch (const tf2::TransformException& ex)
        {
            RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 5000,
                                 "Waiting for %s -> %s: %s", _odom_frame.c_str(),
                                 _base_link_frame.c_str(), ex.what());
            return;
        }

        // Only yaw survives: nav2's planar costmaps and controllers assume the robot
        // moves on the ground plane, so z, roll and pitch are dropped rather than
        // passed through.
        ...
        odom_to_base_footprint.transform.translation.x =
            odom_to_base_link.transform.translation.x;
        odom_to_base_footprint.transform.translation.y =
