...
namespace perseus_vision
{
    namespace
    {
        ...
        // FIX: This must match the static imgsz used for ONNX export.
        // The supplied model was exported with imgsz=320.
        constexpr int MODEL_INPUT_SIZE_PX = 320;
        ...
        /// @brief Edge length of the rock drawn for each detection in rviz, in metres.
        constexpr double ROCK_MARKER_SIZE_M = 0.12;
        /// @brief How high above the rock its text label floats, in metres.
        constexpr double TEXT_MARKER_HEIGHT_OFFSET_M = 0.12;
        /// @brief Cap height of the rock text labels, in metres.
        constexpr double TEXT_MARKER_SCALE_M = 0.08;
        /// @brief How long rviz keeps a rock marker before treating it as stale, in seconds.
        constexpr double MARKER_LIFETIME_S = 0.2;
        /// @brief Opacity of the rock markers drawn in rviz.
        constexpr float MARKER_ALPHA = 0.85f;
        /// @brief rviz namespace holding the rock markers.
        const std::string ROCK_MARKER_NAMESPACE = "rock_depth";
        /// @brief rviz namespace holding the rock text labels.
        const std::string ROCK_TEXT_MARKER_NAMESPACE = "rock_depth_text";
        ...
        /// @brief Builds the marker that clears the previous frame's rock markers from rviz.
        /// @param header Header stamped onto the marker.
        /// @return A DELETEALL marker for the rock namespace.
        visualization_msgs::msg::Marker make_clear_marker(const std_msgs::msg::Header& header)
        {
            visualization_msgs::msg::Marker marker;
            marker.header = header;
            marker.ns = ROCK_MARKER_NAMESPACE;
            marker.id = 0;
            marker.action = visualization_msgs::msg::Marker::DELETEALL;
            return marker;
        }

        /// @brief Builds the rviz rock drawn at a detected rock's estimated pose.
        /// @param header Header stamped onto the marker.
        /// @param marker_id Unique ID within the rock marker namespace.
        /// @param pose Estimated rock pose in the marker's frame.
        /// @param class_id rock class, used to colour the marker.
        /// @return The rock marker.
        visualization_msgs::msg::Marker make_rock_marker(
            ...
            int32_t class_id)
        {
            ...
            marker.pose = pose;
            marker.scale.x = ROCK_MARKER_SIZE_M;
            marker.scale.y = ROCK_MARKER_SIZE_M;
            marker.scale.z = ROCK_MARKER_SIZE_M;
            marker.color.a = MARKER_ALPHA;
            marker.lifetime = rclcpp::Duration::from_seconds(MARKER_LIFETIME_S);
            return marker;
        }

        /// @brief Builds the rviz text label floating above a detected rock.
        /// @param header Header stamped onto the marker.
        /// @param marker_id Unique ID within the rock text marker namespace.
        /// @param pose Estimated rock pose in the marker's frame.
        /// @param class_id rock class, used to name the rock in the label.
            ...
            int32_t class_id)
        {
            visualization_msgs::msg::Marker marker;
            marker.header = header;
            marker.ns = ROCK_TEXT_MARKER_NAMESPACE;
            marker.id = marker_id;
    ...
    RockDetector::RockDetector(const rclcpp::NodeOptions& options)
        : Node("rock_detector", options),
          _ort_env(ORT_LOGGING_LEVEL_WARNING, "rock_detector")
    {
        ...
        // Subscribers
        // Camera drivers commonly publish with best-effort sensor QoS.
        _image_subscription = create_subscription<sensor_msgs::msg::Image>(
            camera_topic, rclcpp::SensorDataQoS(),
            std::bind(&RockDetector::_image_callback, this, std::placeholders::_1));

        _camera_info_subscription = create_subscription<sensor_msgs::msg::CameraInfo>(
            camera_info_topic, rclcpp::SensorDataQoS(),
            std::bind(&RockDetector::_camera_info_callback, this, std::placeholders::_1));

        if (_depth_estimation_mode == DEPTH_MODE_DEPTH_IMAGE)
        {
            _depth_image_subscription = create_subscription<sensor_msgs::msg::Image>(
                _depth_topic, rclcpp::SensorDataQoS(),
                std::bind(&RockDetector::_depth_image_callback, this, std::placeholders::_1));
            _depth_camera_info_subscription = create_subscription<sensor_msgs::msg::CameraInfo>(
                _depth_info_topic, rclcpp::SensorDataQoS(),
                std::bind(&RockDetector::_depth_camera_info_callback, this, std::placeholders::_1));
        }
        ...
        _detect_objects_service = create_service<DetectObjects>(
            "~/detect_objects",
            std::bind(&RockDetector::_handle_detect_objects_request, this,
                      std::placeholders::_1, std::placeholders::_2));

        RCLCPP_INFO(get_logger(), "RockDetectorNode ready — subscribed to %s", camera_topic.c_str());
    ...
    void RockDetector::_load_parameters(std::string& camera_topic_out, std::string& camera_info_topic_out)
    {
        ...
        _param_callback_handle = add_on_set_parameters_callback(
            std::bind(&RockDetector::_parameter_callback, this, std::placeholders::_1));
    }

    std::string RockDetector::_resolve_model_path() const
    {
    ...
    void RockDetector::_image_callback(const sensor_msgs::msg::Image::SharedPtr msg)
    {
    ...
    bool RockDetector::_should_run_inference_now()
    {
    ...
    void RockDetector::_camera_info_callback(const sensor_msgs::msg::CameraInfo::SharedPtr msg)
    {
    ...
    void RockDetector::_depth_image_callback(const sensor_msgs::msg::Image::SharedPtr msg)
    {
        std::lock_guard<std::mutex> lock(_depth_mutex);
        _latest_depth_image = msg;
    }

    void RockDetector::_depth_camera_info_callback(const sensor_msgs::msg::CameraInfo::SharedPtr msg)
    {
        std::lock_guard<std::mutex> lock(_depth_mutex);
        _depth_fx = msg->k[0];
        _depth_fy = msg->k[4];
    ...
    std::vector<float> RockDetector::_preprocess(const cv::Mat& bgr_image)
    {
    ...
    std::vector<detection_t> RockDetector::_postprocess(const float* data, std::size_t num_boxes) const
    {
    ...
    void RockDetector::_run_inference_pipeline(
        ...
        const std_msgs::msg::Header& header)
    {
        ...
        // Validate the detection-model tensor before indexing it. This parser
        // expects raw YOLO detection output [1, 4 + NUM_CLASSES, number_of_boxes].
        if (output_tensors.empty())
        {
            RCLCPP_ERROR(get_logger(), "ONNX model returned no output tensors");
            return;
        }

        const auto output_shape = output_tensors[0].GetTensorTypeAndShapeInfo().GetShape();
        const int64_t expected_values =
            static_cast<int64_t>(BOX_COORDINATE_COUNT + NUM_CLASSES);
        if (output_shape.size() != 3 || output_shape[0] != 1 ||
            output_shape[1] != expected_values || output_shape[2] <= 0)
        {
            std::ostringstream shape_stream;
            for (const int64_t dimension : output_shape)
            {
                shape_stream << dimension << ' ';
            }
            RCLCPP_ERROR(
                get_logger(),
                "Unexpected ONNX output shape [%s]; expected [1, %ld, N]. "
                "Use a detection model exported with nms=False, not a segmentation model.",
                shape_stream.str().c_str(), static_cast<long>(expected_values));
            return;
        }
        ...
        // Non-const because GrabCut adds a mask, contour and centroid.
        std::vector<detection_t> detections =
            _postprocess(output_data, num_boxes);
        for (auto& detection : detections)
        {
            if (!_create_rock_mask(bgr_image, detection))
            {
                // Fallback if GrabCut cannot form a usable mask.
                detection.centroid = cv::Point2f(
                    static_cast<float>(
        ...
        // The separate DetectionOverlay node consumes DetectionArray; do not
        // call an undeclared duplicate overlay publisher here.
    }

    std::string RockDetector::_build_depth_summary_message(std::size_t detected_count, std::size_t resolved_count)
    {
        if (detected_count == 0)
        {
            return "No rock detections found in the latest image.";
        }
        ...
        if (resolved_count == 0)
        {
            message_stream << "Detected " << detected_count
                           << " rock(s) in 2D, but depth estimation failed for all detections. "
                           << "Returning 0 depth-resolved rock pose(s).";
        ...
        if (resolved_count < detected_count)
        {
            message_stream << "Resolved " << resolved_count << " of " << detected_count
                           << " detected rock(s) to depth-derived poses.";
            return message_stream.str();
        }

        message_stream << "Resolved all " << resolved_count << " detected rock(s) to depth-derived poses.";
        return message_stream.str();
    }

    void RockDetector::_publish_detections(
        ...
        const std_msgs::msg::Header& header)
    {
        ...
        for (const auto& detection : detections)
        {
            ...
            detection_msg.class_label = CLASS_NAMES[class_index];
            // One-based class identifier; all rocks use class 1 in this model.
            ...
            if (use_depth)
            {
                ...
                if (_estimate_rock_pose_from_depth(detection, pose, distance_m))
                {
        ...
        {
            message = "No rock detections found in the latest image.";
        }
        else
        {
            std::ostringstream message_stream;
            message_stream << "Detected " << detections.size()
                           << " rock(s) in 2D only; 3D poses are unavailable in the current depth mode.";
    ...
    void RockDetector::_warn_if_depth_frame_mismatched(const std_msgs::msg::Header& color_header)
    {
        ...
        RCLCPP_WARN_THROTTLE(
            ...
            "Depth image frame '%s' differs from colour image frame '%s'. Rock poses are "
    ...
    void RockDetector::_publish_markers(const perseus_interfaces::msg::DetectionArray& detections)
    {
        ...
        for (const auto& detection : detections.detections)
        {
            ...
            marker_array.markers.push_back(
                make_rock_marker(detections.header, marker_id++, detection.pose, class_id));
            marker_array.markers.push_back(
                make_text_marker(detections.header, marker_id++, detection.pose, class_id));
        }

        _rock_marker_publisher->publish(marker_array);
    }
    ...
    // Produce an approximate mask using GrabCut, not a trained segmentation mask.
    bool RockDetector::_create_rock_mask(
        const cv::Mat& bgr_image,
        detection_t& detection) const
    {
        if (bgr_image.empty() ||
            detection.bounding_box.width < 3 ||
            detection.bounding_box.height < 3)
        {
            return false;
        }

        const cv::Rect image_bounds(0, 0, bgr_image.cols, bgr_image.rows);
        // FIX: Declare and clip the GrabCut seed rectangle to the source image.
        cv::Rect grabcut_rect = detection.bounding_box & image_bounds;
        grabcut_rect.x = std::max(1, grabcut_rect.x);
        grabcut_rect.y = std::max(1, grabcut_rect.y);
        grabcut_rect.width = std::min(
            grabcut_rect.width,
            bgr_image.cols - grabcut_rect.x - 1);
        grabcut_rect.height = std::min(
            grabcut_rect.height,
            bgr_image.rows - grabcut_rect.y - 1);

        if (grabcut_rect.width < 3 || grabcut_rect.height < 3)
        {
            return false;
        }

        cv::Mat grabcut_labels(
            bgr_image.size(), CV_8UC1, cv::Scalar(cv::GC_BGD));
        cv::Mat background_model;
        cv::Mat foreground_model;

        // FIX: Guard against OpenCV failures caused by unsuitable rectangles/images.
        try
        {
            cv::grabCut(
                bgr_image,
        ...
        catch (const cv::Exception& exception)
        {
            RCLCPP_WARN_THROTTLE(
                get_logger(), *get_clock(), LOG_THROTTLE_MS,
                "GrabCut failed: %s", exception.what());
            return false;
        }

        cv::Mat definite_foreground;
        cv::Mat probable_foreground;
        cv::compare(
            grabcut_labels,
            cv::GC_FGD,
            definite_foreground,
        ...
        detection.contour = *largest;

        cv::Mat largest_mask = cv::Mat::zeros(bgr_image.size(), CV_8UC1);
        ...
        {
            detection.mask.release();
            detection.contour.clear();
            return false;
        }

        detection.centroid.x = static_cast<float>(moments.m10 / moments.m00);
        detection.centroid.y = static_cast<float>(moments.m01 / moments.m00);
        ...
        double& distance_m_out) const
    {
        ...
        if (!detection.mask.empty())
        {
            ...
            v = cvRound(detection.centroid.y);
        }
    ...
    bool RockDetector::_read_depth_meters_at_pixel(
    ...
    double RockDetector::_read_depth_sample(const sensor_msgs::msg::Image& depth_msg, int row, int column) const
    {
    ...
    void RockDetector::_handle_detect_objects_request(
    ...
    void RockDetector::_run_inference_on_cached_frame()
    {
        cv::Mat latest_frame;
        ...
        const perseus_interfaces::msg::DetectionArray& detections) const
    {
        ...
        const auto epoch_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
                                  ...
                                  .count();
        const std::string filename = save_directory + "/rock_detections_" + std::to_string(epoch_ms) + ".png";
        ...
        RCLCPP_INFO(get_logger(), "Saved rock detection capture: %s", filename.c_str());
    }

    rcl_interfaces::msg::SetParametersResult RockDetector::_parameter_callback(
