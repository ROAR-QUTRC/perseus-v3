...
#include "vision/aruco_detector/aruco_detector.hpp"
...
ArucoDetector::ArucoDetector(const rclcpp::NodeOptions &options)
  ...
  _detection_publisher =
      this->create_publisher<interfaces::msg::DetectionArray>(
...
std::vector<marker_coordinate_t> ArucoDetector::_estimate_marker_poses(
  ...
  for (std::size_t i = 0; i < ids.size(); ++i) {
    ...
    interfaces::msg::Detection detection;
...
RCLCPP_COMPONENTS_REGISTER_NODE(vision::ArucoDetector)
