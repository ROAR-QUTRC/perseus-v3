...
/// @file rock_detector.hpp
/// @brief Rock detection and depth-based pose estimation ROS 2 node.
...
namespace perseus_vision
{
    ...
    /// @brief Human-readable name for the rock class.
    inline const std::array<std::string, NUM_CLASSES> CLASS_NAMES = {"rock"};

    /// @brief Annotation colour of the rock in the image.
    inline const std::array<cv::Scalar, NUM_CLASSES> CLASS_COLORS = {
        cv::Scalar(255, 0, 255),  // Purple in BGR, distinct from natural rock colours.
    };

    /// @brief One rock found by the detection model in a single image.
    struct detection_t
    {
        ...
        /// @brief Bounding box in original camera image coordinates.
        cv::Rect bounding_box{};
        /// @brief Binary mask in the original image coordinates, where rock pixels are 255, and background is 0.
        cv::Mat mask{};
        /// @brief Centre of the segmented rock in image coordinates.
        cv::Point2f centroid{};
        /// @brief Contour points of the segmented rock in the image coordinates.
        std::vector<cv::Point> contour{};
    };

    /// @brief ROS 2 node for detecting coloured rocks and estimating their poses.
    ///
    /// Subscribes to RGB images, runs ONNX-based object detection to find rocks,
    ...
    public:
        ...
        explicit RockDetector(const rclcpp::NodeOptions& options = rclcpp::NodeOptions());
    ...
    private:
        ...
        /// @brief Default furthest depth reading accepted as a valid rock distance, in metres.
        static constexpr double DEFAULT_DEPTH_MAX_RANGE_M = 5.0;
        /// @brief Default nearest depth reading accepted as a valid rock distance, in metres.
        ...
        /// @param color_header Header of the colour image message.
        void _warn_if_depth_frame_mismatched(
            const std_msgs::msg::Header& color_header);

        /// @brief Builds the human-readable summary returned by the detection service.
        /// @param detected_count Number of rocks found in the image.
        /// @param resolved_count Number of those rocks that got a depth-derived pose.
        ...
        /// @param detection Detection to receive the mask, contour, and centroid.
        /// @return True when a usable foreground contour was produced.
        bool _create_rock_mask(
            const cv::Mat& bgr_image,
            detection_t& detection) const;

        /// @brief Estimates a rock pose from its mask centroid, falling back to box centre.
        /// @param detection Detection to estimate a pose for.
        /// @param pose_out Receives the pose in the depth camera frame.
        /// @param distance_m_out Receives the straight-line distance to the rock, in metres.
        /// @return True if a valid, in-range depth reading was found.
        bool _estimate_rock_pose_from_depth(
