    ...
    for (i = 0; i < m; i++)
    {
        ...
        for (ll = 0; ll < m; ll++)
            if (ll != icol)
            {
                dumb = A.val[ll][icol];
                A.val[ll][icol] = 0.0;
                for (l = 0; l < m; l++) A.val[ll][l] -= A.val[icol][l] * dumb;
                for (l = 0; l < n; l++) B.val[ll][l] -= B.val[icol][l] * dumb;
            }
    }
...
// index[1..n] is an output vector that records the row permutation effected by the partial
