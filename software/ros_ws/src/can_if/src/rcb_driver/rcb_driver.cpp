...
RcbDriver::RcbDriver(const rclcpp::NodeOptions &options)
  ...
  try {
    _can_interface.emplace(
        RawCanInterface(this->declare_parameter("can_bus", "can0")));
    _packet_manager.emplace(_can_interface.value());

    _packet_manager->set_callback(
        addressing::filter_t(
            power::SYSTEM_ID, power::control::SUBSYSTEM_ID,
            static_cast<uint8_t>(power::control::device::ROVER_CONTROL_BOARD)),
        {.data_callback = [this](const hi_can::Packet &packet) {
          this->_can_to_ros(packet);
        }});
  } catch (const std::exception &e) {
    RCLCPP_FATAL(this->get_logger(), "Failed to initialise CAN bus: %s",
                 e.what());
    return;
  }
  ...
  RCLCPP_INFO(this->get_logger(),
              "Rover control board driver node initialized");
}
...
void RcbDriver::_ros_to_can(std_msgs::msg::String::UniquePtr msg) {
  ...
  try {
    ...
    const address_t address(
        ...
        static_cast<uint8_t>(
            power::control::power_bus::parameter::CONTROL_IMMEDIATE));
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    RCLCPP_ERROR(rclcpp::get_logger("main"), "Error running rcb driver: %s",
                 e.what());
    return 1;
  }
