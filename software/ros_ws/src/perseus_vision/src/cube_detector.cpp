...
std::string default_packaged_model_path() {
  return ament_index_cpp::get_package_share_directory("perseus_vision") +
         "/models/cube_detector_yolob8s.onnx";
}
...
CubeDetector::CubeDetector()
  ...
  declare_parameter("always_on",
                    true); // keep node alive even without subscribers
  ...
  declare_parameter("depth_image.info_topic",
  ...
  declare_parameter("output_detections_topic",
  ...
  _always_on.store(
      get_parameter("always_on").as_bool()); // set initial value of atomic bool
  ...
  _param_callback_handle = add_on_set_parameters_callback(std::bind(
      &CubeDetector::parameter_callback, this, std::placeholders::_1));
  ...
  // ───────────────────────────────────────────────────────────────
  _pub_annotated =
      create_publisher<sensor_msgs::msg::Image>(_output_img_topic, kQosDepth);
  _pub_cube_markers = create_publisher<visualization_msgs::msg::MarkerArray>(
      _output_markers_topic, kQosDepth);
  _pub_cube_detections =
      create_publisher<perseus_interfaces::msg::ObjectDetections>(
          _output_detections_topic, kQosDepth);
  _srv_detect_objects = create_service<DetectObjects>(
      "~/detect_objects",
      std::bind(&CubeDetector::handle_detect_objects_request, this,
...
// ── image callback
// ────────────────────────────────────────────────────────────
void CubeDetector::image_callback(
  ...
  if (processing_frequency_hz > 0.0) {
    ...
    const int64_t min_period_ns =
        static_cast<int64_t>(1e9 / processing_frequency_hz);
...
void CubeDetector::camera_info_callback(
...
void CubeDetector::depth_image_callback(
    const sensor_msgs::msg::Image::SharedPtr msg) {
  std::lock_guard<std::mutex> lock(_depth_mutex);
  _latest_depth_image = msg;
}

void CubeDetector::depth_camera_info_callback(
    const sensor_msgs::msg::CameraInfo::SharedPtr msg) {
  std::lock_guard<std::mutex> lock(_depth_mutex);
  _depth_fx = msg->k[0];
  _depth_fy = msg->k[4];
...
std::vector<float> CubeDetector::preprocess(const cv::Mat &bgr_image) {
  ...
  for (auto &ch : channels)
    blob.insert(blob.end(), reinterpret_cast<float *>(ch.data),
                reinterpret_cast<float *>(ch.data) + 640 * 640);
  return blob;
}
// ── postprocess
// ───────────────────────────────────────────────────────────────
std::vector<Detection> CubeDetector::postprocess(const float *data,
                                                 size_t num_boxes) {
  ...
  for (size_t i = 0; i < num_boxes; ++i) {
    ...
    if (best_score < _confidence_threshold)
      continue;
    // YOLOv8 outputs box coordinates in the letterboxed 640x640 space, so we
    // need to reverse the letterbox transformation to get back to original
    // image coordinates
    const int x1 = std::clamp(
        static_cast<int>((xc - w / 2.0f - _pad_x) / _letterbox_scale), 0,
        _orig_w);
    const int y1 = std::clamp(
        static_cast<int>((yc - h / 2.0f - _pad_y) / _letterbox_scale), 0,
        _orig_h);
...
void CubeDetector::publish_annotated_image(
  ...
  for (const auto &d : detections) {
    ...
    cv::putText(annotated, label, cv::Point(d.bbox.x, d.bbox.y - 20),
                cv::FONT_HERSHEY_SIMPLEX, 0.6, colour, 2);

    // draw center coordinates below class label
    cv::putText(annotated, center_label, cv::Point(d.bbox.x, d.bbox.y - 5),
                cv::FONT_HERSHEY_SIMPLEX, 0.5, colour, 1);

    // draw crosshair at center
    cv::drawMarker(annotated, cv::Point(cx, cy), colour, cv::MARKER_CROSS, 15,
                   1);
  }
...
void CubeDetector::publish_depth_markers(
  ...
  for (const auto &detection : detections) {
    ...
    if (target_frame != source_frame) {
      if (!transform_pose_to_output_frame(pose, target_frame, source_frame,
                                          source_stamp, marker_pose)) {
        continue;
      }
    }
    ...
    cube_marker.color.r =
        static_cast<float>(CLASS_COLOURS[detection.class_id][2] / 255.0);
    cube_marker.color.g =
        static_cast<float>(CLASS_COLOURS[detection.class_id][1] / 255.0);
    cube_marker.color.b =
        static_cast<float>(CLASS_COLOURS[detection.class_id][0] / 255.0);
    ...
    ss << CLASS_NAMES[detection.class_id] << " x:" << std::fixed
       << std::setprecision(2) << marker_pose.position.x << " y:" << std::fixed
       << std::setprecision(2) << marker_pose.position.y;
  ...
  if (detections.empty()) {
    ...
    std::ostringstream message_stream;
    message_stream << "Resolved " << detections_msg.ids.size() << " of "
                   << detections.size()
...
void CubeDetector::publish_2d_detections(
  ...
  _pub_cube_detections->publish(detections_msg);
  {
    ...
    if (detections.empty()) {
      _latest_detection_message =
          "No cube detections found in the latest image.";
    } else {
      std::ostringstream message_stream;
...
bool CubeDetector::estimate_cube_pose_from_depth(
    const Detection &detection, geometry_msgs::msg::Pose &pose_out,
    ...
    for (int du = -radius; du <= radius; ++du) {
      ...
      if (depth_msg.encoding == "16UC1") {
        const auto *row = reinterpret_cast<const uint16_t *>(
            &depth_msg.data[yy * depth_msg.step]);
...
CubeDetector::quaternion_from_yaw_pitch(double yaw, double pitch) const {
...
bool CubeDetector::transform_pose_to_output_frame(
  ...
  try {
    const geometry_msgs::msg::PoseStamped out_pose =
        _tf_buffer->transform(in_pose, target_frame);
...
void CubeDetector::run_inference_pipeline(const cv::Mat &bgr_image,
                                          const std_msgs::msg::Header &header) {
  ...
  Ort::Value input_tensor =
      Ort::Value::CreateTensor<float>(mem_info, blob.data(), blob.size(),
                                      input_shape.data(), input_shape.size());

  const char *input_names[] = {_input_name.c_str()};
  const char *output_names[] = {_output_name.c_str()};

  auto output_tensors = _ort_session->Run(Ort::RunOptions{nullptr}, input_names,
...
void CubeDetector::handle_detect_objects_request(
  ...
  if (!_always_on.load()) {
    ...
    std_msgs::msg::Header latest_header;
    {
      ...
      if (!_has_latest_bgr_frame || _latest_bgr_frame.empty()) {
        RCLCPP_WARN(get_logger(), "detect_objects requested, but no camera "
  ...
  std::strftime(timestamp_str, sizeof(timestamp_str), "%Y-%m-%d %H:%M:%S",
                &local_tm);
  ...
  int y = 50;
  cv::putText(frame_to_save, std::string("Frame: ") + frame_id_copy,
  ...
  for (size_t i = 0; i < ids_copy.size() && i < poses_copy.size(); ++i) {
    ...
    line << "ID " << ids_copy[i] << " x=" << std::fixed << std::setprecision(2)
         << poses_copy[i].position.x << " y=" << std::fixed
         << std::setprecision(2) << poses_copy[i].position.y;
    cv::putText(frame_to_save, line.str(), cv::Point(10, y),
                cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 255, 0), 1);
  }

  const std::string filename =
      save_dir + "/cube_detections_" + std::to_string(epoch_ms) + ".png";
