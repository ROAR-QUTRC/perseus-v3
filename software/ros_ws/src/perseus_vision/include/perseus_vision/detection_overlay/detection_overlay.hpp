...
private:
  ...
  static inline const std::vector<std::string> DEFAULT_DETECTION_TOPICS = {
      "/perseus_vision/aruco/detections", "/perseus_vision/cube/detections"};
  ...
  std::vector<
      rclcpp::Subscription<perseus_interfaces::msg::DetectionArray>::SharedPtr>
      _detection_subscriptions;

  // Latest detections per source topic
  std::mutex _detections_mutex;
  std::unordered_map<std::string, perseus_interfaces::msg::DetectionArray>
      _latest_detections;
};
