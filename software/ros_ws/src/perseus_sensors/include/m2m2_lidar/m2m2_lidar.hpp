...
private:
  ...
  static constexpr size_t INTERPOLATED_POINTS =
      4096; // Number of points per scan
  // for imu
   ...
   */
  [[nodiscard]] static bool is_within_epsilon(float a, float b,
                                              float epsilon = EPSILON);

  // Network communication methods
  [[nodiscard]] bool
  _send_json_request(const std::string &command,
  ...
  [[nodiscard]] std::vector<std::tuple<float, float, bool>>
  _decode_laser_points(const std::string &base_64_encoded);
  [[nodiscard]] std::vector<std::tuple<float, float, bool>> _interpolate_points(
      const std::vector<std::tuple<float, float, bool>> &points);
  [[nodiscard]] std::vector<uint8_t>
  _create_config_command(const sensor_config_t &config);
