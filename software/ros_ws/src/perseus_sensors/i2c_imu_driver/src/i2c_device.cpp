...
bool is_valid_i2c_path(const std::string &path) {
  ...
  // Must be digits only
  if (bus_num.empty() ||
      !std::all_of(bus_num.begin(), bus_num.end(), ::isdigit)) {
    return false;
  }
          ...
          [this]() -> int {
            ...
            if (!is_valid_i2c_path(_bus_path)) {
              throw std::invalid_argument(
                  "Invalid I2C bus path: " + _bus_path +
...
bool I2cDevice::is_connected() const { return _i2c_fd.get() >= 0; }
...
bool I2cDevice::read_registers(uint8_t reg_address, uint8_t *buffer,
                               size_t length) {
  // Maximum I2C transaction size (conservative limit for compatibility)
  constexpr size_t MAX_I2C_TRANSACTION_SIZE = 255;

  if (!is_connected() || buffer == nullptr || length == 0 ||
      length > MAX_I2C_TRANSACTION_SIZE) {
    return false;
  }
...
bool I2cDevice::write_registers(uint8_t reg_address, const uint8_t *buffer,
                                size_t length) {
  // Maximum I2C transaction size (conservative limit for compatibility)
  constexpr size_t MAX_I2C_TRANSACTION_SIZE = 255;
