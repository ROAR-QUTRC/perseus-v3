...
// LSM6DSOX Constants
const double LSM6DSOX_ACCEL_SCALE_FACTOR =
    0.061e-3 * 9.81; // ±2g range: 0.061 mg/LSB to m/s²
const double LSM6DSOX_GYRO_SCALE_FACTOR =
    8.75e-3 * M_PI / 180.0; // ±250 dps range: 8.75 mdps/LSB to rad/s
...
const double MPU_ACCEL_SCALE_FACTOR =
    9.81 / 16384.0; // ±2g range: 16384 LSB/g to m/s²
const double MPU_GYRO_SCALE_FACTOR =
    (250.0 * M_PI / 180.0) / 32768.0;             // ±250 dps range to rad/s
...
const double LSM6DS3_ACCEL_SCALE_FACTOR =
    0.061e-3 * 9.81; // ±2g range: 0.061 mg/LSB to m/s²
const double LSM6DS3_GYRO_SCALE_FACTOR =
    8.75e-3 * M_PI / 180.0; // ±250 dps range: 8.75 mdps/LSB to rad/s
const double LSM6DS3_TEMP_SCALE_FACTOR = 1.0 / 256.0; // 256 LSB/°C
const double LSM6DS3_TEMP_OFFSET = 25.0;              // 25°C offset

std::unordered_map<std::string, ImuDeviceConfig>
    ImuDeviceRegistry::_device_configs;
...
std::optional<std::reference_wrapper<const ImuDeviceConfig>>
ImuDeviceRegistry::get_device_config(const std::string &device_name) {
...
std::string ImuDeviceRegistry::get_default_device() { return "lsm6dsox"; }
