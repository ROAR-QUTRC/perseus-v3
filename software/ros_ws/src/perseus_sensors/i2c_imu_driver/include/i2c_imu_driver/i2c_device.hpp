 ...
 *
 * This class provides a clean interface for I2C communication using Linux I2C
 * API. It manages the I2C device file descriptor and provides methods for
 * device detection, register reading/writing, and proper error handling.
 */
class I2cDevice {
public:
  /**
   ...
   *
   * @note Uses hardware I2C timeout (configured at bus level, typically 1-2
   * seconds)
   */
   ...
   *
   * Performs an I2C combined transaction: write register address, then read
   * data.
   *
   ...
   *
   * @param reg_address Starting register address
   * @param buffer Buffer containing data to write (must be at least @p length
   * bytes)
   ...
   * @threadsafe NO - Must not be called from multiple threads concurrently
   */
  bool write_registers(uint8_t reg_address, const uint8_t *buffer,
                       size_t length);
