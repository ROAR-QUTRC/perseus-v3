 ...
 * @details This class provides a centralized registry for different IMU device
 * configurations. It maintains device-specific settings including register
 * addresses, scale factors, and configuration values for various IMU sensors
 * (e.g., MPU6050, ICM20948). The registry follows a singleton pattern and lazy
 * initialization to ensure device configurations are loaded only when needed.
 *
 * Example usage:
 * @code
...
public:
   ...
   * @return Device configuration wrapped in std::optional, or std::nullopt if
   * not found
   */
  static std::optional<std::reference_wrapper<const ImuDeviceConfig>>
  get_device_config(const std::string &device_name);
