 ...
 *    - Loads ROS params (IP, port, topics, frame_id) - likely to come from a
 * launch file in future
 ...
 *    - Releases resources
 *
 * This is under construction but hopefully helpful as multiple people are
 ...
 * Individual data points in the scan are not timestamped but are assumed to be
 * evenly spaced and have a time_increment value. scan_time is the time for a
 * one complete 360 degree sweep.
 *
 * *
...
M2M2Lidar::M2M2Lidar(const rclcpp::NodeOptions &options)
  ...
  // {
  //     RCLCPP_ERROR(this->get_logger(), "Failed to set logger level to
  //     DEBUG");
  // }
  // else
  // {
  ...
  socket_config_handlers_t handlers{
      ...
      .pre_connect = [this](int fd) -> bool {
        ...
        struct timeval timeout_value{.tv_sec = 5, .tv_usec = 0};

        if (setsockopt(fd, SOL_SOCKET, SO_RCVTIMEO, &timeout_value,
                       sizeof(timeout_value)) < 0) {
          RCLCPP_ERROR(this->get_logger(), "Failed to set receive timeout: %s",
                       strerror(errno));
          return false;
        }

        if (setsockopt(fd, SOL_SOCKET, SO_SNDTIMEO, &timeout_value,
                       sizeof(timeout_value)) < 0) {
          RCLCPP_ERROR(this->get_logger(), "Failed to set send timeout: %s",
                       strerror(errno));
          return false;
        }

        int keepalive_enabled = 1;
        if (setsockopt(fd, SOL_SOCKET, SO_KEEPALIVE, &keepalive_enabled,
                       sizeof(keepalive_enabled)) < 0) {
          RCLCPP_ERROR(this->get_logger(), "Failed to set keepalive: %s",
                       strerror(errno));
          return false;
        }
  ...
  try {
    ...
    _client.emplace(address_t{.hostname = sensor_ip,
                              .service = std::to_string(sensor_port)},
                    socket_protocol::TCP, address_t{}, handlers);
    RCLCPP_INFO(this->get_logger(), "Network client created successfully");
  } catch (const std::exception &e) {
    RCLCPP_ERROR(this->get_logger(), "Failed to create network client: %s",
                 e.what());
    throw;
  }
  ...
  nlohmann::json request = {{"command", "getlaserscan"},
                            {"request_id", _request_id++}};
  std::string full_message = request.dump() + "\r\n\r\n";
  RCLCPP_DEBUG(this->get_logger(), "Sending test request: %s",
               full_message.c_str());

  if (_send_json_request("getlaserscan")) {
    auto test_response = _receive_json_response();
    if (test_response.empty()) {
      RCLCPP_WARN(this->get_logger(), "No response received from sensor during "
                                      "init test, proceeding anyway");
    } else if (!test_response.contains("result") ||
               !test_response["result"].contains("code") ||
               test_response["result"]["code"] != 1) {
      RCLCPP_WARN(this->get_logger(),
                  "Invalid response during init test: %s, proceeding anyway",
                  test_response.dump().c_str());
    } else {
      RCLCPP_DEBUG(this->get_logger(),
                   "Connection test successful, configuring sensor...");
    }
  } else {
    RCLCPP_WARN(
        this->get_logger(),
        "Failed to send initial getlaserscan command, proceeding anyway");
  }
  ...
  if (_read_imu) {
    ...
    _imu_timer = this->create_wall_timer(initial_imu_period, [this]() {
    ...
    RCLCPP_INFO(this->get_logger(),
                "IMU reading disabled by parameter 'read_imu'");
  }
...
vector<uint8_t> M2M2Lidar::_decode_base64(const string &encoded) {
  ...
  if (!context) {
    RCLCPP_ERROR(this->get_logger(),
                 "Failed to create Base64 decoding context");
  ...
  try {
    ...
    int output_length;
    int result = EVP_DecodeUpdate(
        context, decoded.data(), &output_length,
    ...
    int final_length;
    result =
        EVP_DecodeFinal(context, decoded.data() + output_length, &final_length);
...
vector<std::tuple<float, float, bool>>
M2M2Lidar::_decode_laser_points(const string &base_64_encoded) {
  ...
  while (pos < decoded.size()) {
    ...
    if (b == sentinel1) {
      if (pos + 2 < decoded.size() && decoded[pos + 1] == 0 &&
...
bool M2M2Lidar::_send_json_request(const std::string &command,
                                   const nlohmann::json &args) {
  assert(_client && "Network client should be initialised by constructor");

  // RCLCPP_INFO(this->get_logger(), "Preparing to send command: %s",
  // command.c_str());

  // Create the JSON request structure
  nlohmann::json request = {{"command", command},
                            {"request_id", _request_id++}};
  ...
  RCLCPP_DEBUG(this->get_logger(), "Full message to send: %s",
               full_message.c_str());
  ...
  try {
    ...
    if (sent != static_cast<ssize_t>(full_message.length())) {
      RCLCPP_ERROR(
          this->get_logger(),
          "Failed to send complete message: only sent %zd of %zu bytes", sent,
          full_message.length());
      return false;
    }
...
nlohmann::json M2M2Lidar::_receive_json_response() {
  ...
  while (!found_delim) {
    ...
    if (now - start_time > timeout) {
      if (received.empty()) {
        RCLCPP_INFO(
            this->get_logger(),
            "No data received from sensor after %ld seconds from "
            "initialisation",
            std::chrono::duration_cast<std::chrono::seconds>(timeout).count());
      } else {
        RCLCPP_ERROR(this->get_logger(),
                     "Timeout waiting for response delimiter. Received %zu "
                     "bytes: %.100s",
                     received.size(),
    ...
    try {
      ...
      if (!chunk) {
        RCLCPP_DEBUG(this->get_logger(),
                     "No data available yet in this iteration");
      ...
      RCLCPP_DEBUG(this->get_logger(),
                   "Current buffer size: %zu bytes, content: %s",
                   received.size(),
                   std::string(received.begin(), received.end()).c_str());
    } catch (const std::exception &e) {
      RCLCPP_ERROR(this->get_logger(), "Exception during receive: %s",
                   e.what());
    ...
    if (received.size() >= 4) {
      std::string_view last_four(
          reinterpret_cast<const char *>(&received[received.size() - 4]), 4);
      if (last_four == "\r\n\r\n") {
        found_delim = true;
        RCLCPP_DEBUG(this->get_logger(), "Found delimiter after %zu bytes",
                     received.size());
  ...
  try {
    ...
    RCLCPP_DEBUG(this->get_logger(), "Parsed JSON: %s",
                 json_response.dump().c_str());
    return json_response;
  } catch (const std::exception &e) {
    RCLCPP_ERROR(this->get_logger(), "Failed to parse JSON response: %s",
                 e.what());
    RCLCPP_DEBUG(this->get_logger(), "Raw data that failed parsing: %s",
                 json_str.c_str());
    return nlohmann::json();
  }
}
 ...
 *
 * @details Constructs a byte sequence following the sensor's protocol format
 * for configuration commands. Note that this function may become deprecated as
 * hardware testing progresses - the m2m2 LIDAR's default configuration may be
 * sufficient without manual configuration. Testing will confirm or not.
 *
 */
vector<uint8_t>
M2M2Lidar::_create_config_command(const sensor_config_t &config) {
  ...
  double clamped_frequency =
      std::clamp(config.scan_frequency, MIN_SCAN_FREQUENCY, MAX_SCAN_FREQUENCY);
  ...
  // Add configuration parameters if needed - this might go-away post hardware
  // testing
...
void M2M2Lidar::_send_command(const std::vector<uint8_t> &command) {
  ...
  RCLCPP_DEBUG(this->get_logger(), "Successfully sent command of %zu bytes",
               command.size());
}
...
void M2M2Lidar::_read_sensor_data() {
  ...
  if (response.empty() || response["result"].is_null() ||
      !response["result"].contains("laser_points") ||
      response["result"]["code"] != 1) // 1 is success code
  {
    RCLCPP_ERROR(this->get_logger(),
                 "Failed to receive valid laser scan response");
    return;
  }
  ...
  RCLCPP_DEBUG(this->get_logger(), "Publishing scan with %zu ranges",
               scan.ranges.size());
...
std::vector<std::tuple<float, float, bool>> M2M2Lidar::_interpolate_points(
  ...
  for (const auto &point : input_points) {
     ...
     *
     * @details This normalization is crucial for consistent LIDAR point
     * processing:
  ...
  for (size_t i = 0; i < INTERPOLATED_POINTS; ++i) {
    ...
    if (auto found_iterator =
            std::ranges::find_if(points | std::views::drop(source_point_index),
    ...
    if (source_point_index == 0) {
      ...
      interpolated.emplace_back(target_angle, get<1>(points.back()),
                                get<2>(points.back()));
    } else {
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    RCLCPP_ERROR(rclcpp::get_logger("main"), "Error running M2M2Lidar: %s",
                 e.what());
    return 1;
  }
