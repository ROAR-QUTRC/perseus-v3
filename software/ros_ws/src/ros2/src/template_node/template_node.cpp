...
TemplateNode::TemplateNode(const rclcpp::NodeOptions &options)
  ...
  _templateSubscription = this->create_subscription<std_msgs::msg::UInt8>(
      "subscribed_topic_name", 10,
      std::bind(&TemplateNode::_templateCallback, this, std::placeholders::_1));
  _templateTimer = this->create_wall_timer(
      TIMER_PERIOD, std::bind(&TemplateNode::_timerCallback, this));
  _templatePublisher = this->create_publisher<std_msgs::msg::UInt16>(
      "publishing_topic_name", 10);
...
void TemplateNode::_templateCallback(const std_msgs::msg::UInt8 msg) {
  RCLCPP_DEBUG(
      this->get_logger(),
      "Callback function - Received something from subscribed_topic_name");
  ...
  RCLCPP_INFO(this->get_logger(),
              "Sent received data to publishing_topic_name");
}

// Runs every TIMER_PERIOD (100ms)
void TemplateNode::_timerCallback(void) {
  RCLCPP_INFO(this->get_logger(), "Last data received was %d",
              _lastMsgReceived);
}
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    RCLCPP_ERROR(rclcpp::get_logger("main"), "Error running Template Node: %s",
                 e.what());
    return 1;
  }
