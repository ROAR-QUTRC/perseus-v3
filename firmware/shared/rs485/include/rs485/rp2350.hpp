// rp2350.hpp
//
// RP2350 backend for rs485::Port: hardware UART for RX, a PIO program for TX,
// and a plain GPIO for the transceiver's direction (DE/~RE) pin.
...
namespace rs485
{
    ...
    public:
        ...
        Rp2350Port(uart_inst_t* uart, uint rx_pin, uint dir_pin, PIO tx_pio, uint tx_sm,
        ...
        // Reads up to `cap` bytes: waits up to `first_byte_timeout_us` for the
        // first byte, then keeps reading until `frame_gap_us` passes with no
        ...
        // made rather than only once a first byte arrives. Fine for a
