...
bool EncoderBus::begin()
{
    ...
    for (size_t i = 0; i < kEncoderCount; ++i)
    {
        ...
        config.slave = kPlacement[i].slave;
        ...
        if (!buses_[kPlacement[i].bus]->mbus.add_device(enc::make_device(config)))
        {
            ESP_LOGE(TAG, "failed to register %s", kNames[i]);
            return false;
        }
    }
