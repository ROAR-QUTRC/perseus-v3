...
CRASH_CONSECUTIVE_FAILURES = 10  # genuine no-response timeouts in a row -> "probably crashed"
...
def action_read_angle(client, addr):
    outcome, _ = classify(
        lambda: client.read_holding_registers(address=REG_ANGLE_RAW, count=2, device_id=addr)
    )
...
def action_read_status(client, addr):
    outcome, _ = classify(
        lambda: client.read_holding_registers(address=REG_STATUS, count=1, device_id=addr)
    )
    return outcome
...
def action_read_discovery(client, addr):
    outcome, _ = classify(
        lambda: client.read_holding_registers(address=REG_DISCOVERY, count=1, device_id=addr)
    )
    return outcome
...
def action_toggle_discovery(client, addr):
    ...
    outcome, _ = classify(
        lambda: client.write_register(address=REG_DISCOVERY, value=value, device_id=addr)
    )
    if outcome != "ok":
        return outcome
...
def action_zero_command(client, addr):
    value = random.choice([0, 1])  # exercises both "real zero" and the "0 = no-op" contract
    outcome, _ = classify(
        lambda: client.write_register(address=REG_ZERO_COMMAND, value=value, device_id=addr)
    )
    return outcome
...
def action_write_multiple(client, addr):
    ...
    outcome, _ = classify(
        lambda: client.write_registers(address=REG_ZERO_COMMAND, values=[0], device_id=addr)
    )
    return outcome
...
def action_read_out_of_range(client, addr):
    outcome, _ = classify(
        lambda: client.read_holding_registers(address=REG_OUT_OF_RANGE, count=1, device_id=addr)
    )
    if outcome == "ok":
...
def action_write_read_only(client, addr):
    ...
    outcome, _ = classify(lambda: client.write_register(address=reg, value=1, device_id=addr))
    if outcome == "ok":
...
def run(client, addrs, duration_s, stats, start):
    ...
    next_report = start + 60  # a 30-minute silent script is indistinguishable from a hung one
...
def print_summary(stats, start, duration_s, addrs):
    ...
    print("=" * 60)
    print(f"duration: {duration_s / 60:.1f} min, boards tested: {addrs}, total requests: {total}")
    ...
    for (addr, action, outcome), count in sorted(counts.items(), key=lambda kv: (str(kv[0][0]), kv[0][1], kv[0][2])):
    ...
    unexpected = sum(c for (_a, _act, o), c in counts.items() if o == "unexpected_failure")
    if crash_events:
...
def main():
    ...
    parser.add_argument(
        "--duration", type=float, default=1800, help="seconds to run (default 1800 = 30 min)"
    )
    parser.add_argument(
        "--debug-port",
        action="append",
    ...
    client = ModbusSerialClient(
        port=args.port, baudrate=BAUD, bytesize=8, parity="N", stopbits=1, timeout=0.3, retries=1
    )
    ...
    if not args.debug_port:
        print("-- no --debug-port given: crash detection relies on Modbus responsiveness alone --")

    print(f"-- stress testing {len(addrs)} board(s) {addrs} for {args.duration / 60:.1f} min --")
